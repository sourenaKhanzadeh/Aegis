    ...
    try:
        ...
        for detector in slither.detectors:
            for issue in detector.issues:
                ...
                line_number = (
                    issue.source_mapping[0].lines[0]
                    if issue.source_mapping[0].lines
                    else "Unknown"
                )
        ...
        for contract in slither.contracts:
            for function in contract.functions_entry_points:
                ...
                for node in function.nodes:
                    if (
                        node.expression
                        and "call" in str(node.expression)
                        and "require" not in str(node.expression)
                    ):
                        findings.append(
                            {
